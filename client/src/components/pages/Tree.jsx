...
const Tree = () => {
  ...
  useEffect(() => {
    ...
    function update() {
      ...
      monkey.x = Phaser.Math.Clamp(monkey.x, -4*windowWidth/3, 4*windowWidth/3);
      ...
      if (this.upKey.isDown && this.physics.overlap(monkey, this.tree)) {
        monkey.y -= windowHeight * (1 / 75); // Move the monkey up when pressing the up key
      }

      if (this.downKey.isDown && this.physics.overlap(monkey, this.tree)) {
        // Prevent the monkey from going past the bottom of the tree or ground
        if (monkey.y < 0) {
          monkey.y += windowHeight * (1 / 75); // Move the monkey down when pressing the down key
        }
      }
